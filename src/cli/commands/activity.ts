...
export async function activityCommand(opts: ActivityOptions): Promise<void> {
  ...
  if (!events.length) {
    ...
    ui.line(ui.dim(reporting.length ? `Hooks are installed for: ${reporting.map((a) => a.name).join(', ')}. Events appear once an agent runs in this project.` : 'Install hooks with `athena agents add claude-code` (or cursor, codex, copilot, gemini-cli) to record agent activity.'));
    return;
  }
