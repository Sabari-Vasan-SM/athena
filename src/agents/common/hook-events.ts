...
function classifyTool(tool: string, command: string | null, files: string[]): Classified {
  ...
  if (READ_TOOLS.has(t)) return { kind: t === 'grep' || t === 'glob' ? 'search' : 'read', state: 'ANALYZING', message: `Reading${where || ` (${tool})`}` };
  if (t === 'bash' || t === 'shell' || t.includes('terminal')) {
...
export function normalizeHookEvent(agent: string, hookName: string, payload: Record<string, unknown>): AgentEvent | null {
  ...
  else if (h === 'userpromptsubmit' || h === 'beforesubmitprompt') c = { kind: 'prompt', state: 'PLANNING', message: 'Received a new task from the developer' };
  else if (h === 'stop' || h === 'afteragentresponse') c = { kind: 'stop', state: 'SUCCESS', message: 'Finished responding' };
  ...
  else if (h === 'pretooluse' || h === 'permissionrequest') c = tool ? classifyTool(tool, command, files) : null;
  else if (h === 'posttooluse' || h === 'posttoolusefailure') return null; // avoid duplicating PreToolUse
